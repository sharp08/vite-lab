...
class Lazy {
  ...
  constructor(options: ILazyOptions) {
    ...
    this.loadingSrc = options.loadingSrc || DEFAULT_URL;
    this.errorSrc = options.errorSrc || DEFAULT_URL;
  ...
  // 创建过程仅执行一次，回调会在 lazy.mounted 之后执行，也会在后续状态发生变化时执行若干次
    ...
    this.observer = new IntersectionObserver(
      entries => {
        entries.forEach(entry => {
          ...
          const manager = this.managerQueue.find(({ el }) => el === target);
          // 如果没找到，则并非指令挂载的元素，直接返回
          if (!manager) return;
          //  只要某个标签出现在可视区，就将其实例标识为 true，此后不再变化
          manager.isIntersecting = true;
  ...
  update(el: HTMLElement, binding: DirectiveBinding) {
    const src = binding.value;
    const manager = this.managerQueue.find(manager => manager.el === el);
    if (manager) {
      manager.update(src);
    }
  }
...
export const LazyLoadDirective = (app: App, options: ILazyOptions = {}) => {
  const lazy = new Lazy(options);

  app.directive("lazy", {
    // 直接 lazy.mounted 调用时，内部的 this 指向 lazy
    // 此处是将方法交给 mounted，this 指向了 undefined，因此需要用 bind 矫正
